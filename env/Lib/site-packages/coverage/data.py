...
class CoverageData(object):
    ...
    * **run information**: information about the program execution.  This is
    ...
    def add_to_hash(self, filename, hasher):
        ...
        """
        if self._has_arcs():
            hasher.update_user(sorted(self.arcs(filename) or []))
        else:
            hasher.update_user(sorted(self.lines(filename) or []))
        hasher.update_user(self.file_tracer(filename))
    ...
    def combine_parallel_data(self, data, aliases=None, data_paths=None, strict=False):
        ...
        for f in files_to_combine:
            ...
            try:
                ...
                data.update_user(new_data, aliases=aliases)
                files_combined += 1
                if self.debug and self.debug.should('dataio'):
